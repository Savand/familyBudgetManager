...
@NamedQueries({
    ...
    @NamedQuery(name = FundsFlow.BY_BUDGET_ID, query = "SELECT f FROM FundsFlow f WHERE f.budget.id=:id ORDER BY f.id"),
    @NamedQuery(name = FundsFlow.BY_BUDGET_ID_BETWEEN_DATES, query = "SELECT f FROM FundsFlow f WHERE f.budget.id=:id AND"
...
public class FundsFlow extends BaseEntity {
  ...
  public FundsFlow(Integer amount, Budget budget, String description, User byUser, LocalDateTime operationDate,
  ...
  public String toString() {
    return "FundsFlow [" + super.toString() + "amount=" + amount + ", description=" + description + ", operationDate="
        + operationDateTime + ", fundsFlowType=" + fundsFlowType + "]";
  }
