...
@Repository
public class JdbcUserRepositoryImpl implements UserRepository {// TODO add handling for user roles. The sole reason for
                                                               // tests failing with jdbc profiling

  private static final BeanPropertyRowMapper<User> ROW_MAPPER = BeanPropertyRowMapper.newInstance(User.class);
  private static final String SELECT_FROM_USERS = "SELECT id, creation_date, last_update, user_name as name, email, password, user_icon, enabled FROM users ";
  ...
  public User save(User user) {
    MapSqlParameterSource map = new MapSqlParameterSource().addValue("id", user.getId())
        .addValue("creation_date", user.getCreationDate()).addValue("user_name", user.getName())
        .addValue("email", user.getEmail()).addValue("password", user.getPassword())
        .addValue("user_icon", user.getUserIcon()).addValue("enabled", user.isEnabled());

    if (user.isNew()) {
      Number id = insertUser.executeAndReturnKey(map);
      user.setId(id.intValue());
    } else {
      namedParameterJdbcTemplate.update("UPDATE users SET last_update= now(), user_name= :user_name, email= :email,"
  ...
  public User getByEmail(String email) {
    List<User> users = jdbcTemplate.query(SELECT_FROM_USERS + "WHERE email=?", ROW_MAPPER, email);
    return DataAccessUtils.singleResult(users);
  }
